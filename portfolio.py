...
def build_website():
    ...
    # If the path doesn't exist (e.g. running on a different machine), use current directory
    ...
    # --- COMPETENCIES ---
    competencies_list = [
        "Ethnographic methods", "Qualitative & quantitative methods", "User involvement",
        "Interviewing", "AI alignment", "UX research", "Human-centered design",
        "Participatory technology assessment & design", "Data scraping", "Network analysis",
        "Data visualization", "Gephi", "Philosophy of technology",
        "Life-cycle assessment (LCA)", "Problem-based learning (PBL)", 
        ...
        <head>
            ...
            <style>
                ...
                .modern-input:focus {{
                    ...
                    box-shadow: 0 0 0 2px #0d9488, 0 4px 6px -1px rgba(0, 0, 0, 0.1);
                    outline: none;
                }}
                .modern-label {{
                    display: block;
                ...
                #runaway-btn {{
                    position: absolute;
                    /* Using transform for high performance animation */
                    transition: transform 0.4s cubic-bezier(0.25, 1, 0.5, 1);
                    
                    /* UPDATED POSITION: Starts at very top of container */
                    left: 1.5rem; 
                    top: 0; 
                }}
            </style>
    ...
    def get_footer():
        ...
        <script>
            // Uses sessionStorage so language preference resets when the tab is closed ("Opening anew" = English)
            ...
            document.addEventListener('DOMContentLoaded', () => {{
                // Default to 'en' if session storage is empty (new session)
    ...
    <body class="bg-stone-50 text-stone-900 pt-20">
        ...
        <main class="max-w-6xl mx-auto px-6">
            <section class="min-h-screen grid md:grid-cols-2 gap-12 items-center pt-10 pb-10">
                <div class="order-2 md:order-1 relative z-10 animate-fade-up">
                    ...
                    <div id="prank-container" class="relative mt-6 w-full max-w-4xl h-96 border border-transparent">
                        <button id="runaway-btn" class="bg-stone-900 text-white px-8 py-4 rounded-full font-mono text-xs uppercase tracking-widest shadow-lg cursor-pointer whitespace-nowrap hover:bg-teal-600">
                ...
                <div class="order-1 md:order-2 flex flex-col justify-center items-center md:items-end animate-fade-up" style="animation-delay: 0.2s;">
                    <h2 class="text-xl font-extrabold text-stone-900 mb-2 tracking-tight self-start md:self-end cursor-default">
                        {user_name}
                    </h2>
                    <div class="group relative w-full max-w-md aspect-square">
                        ...
                        <div class="relative w-full h-full overflow-hidden bg-stone-200 shadow-xl">
                            <img src="images/{profile_image_index}" 
                                 class="w-full h-full object-cover transition-all duration-700 filter grayscale group-hover:grayscale-0 group-hover:scale-105"
                                 alt="Thomas Julsgaard">
                        </div>
                    </div>
                </div>
            </section>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {{
                ...
                // Track button position relative to center (0,0)
                let btnX = 0;
                let btnY = 0;
                ...
                if(btn && container) {{
                    const triggerMove = (mouseX, mouseY) => {{
                        ...
                        // FIX: Disable jump on mobile/touch to prevent scrolling issues
                        ...
                        // Calculate Vector from Mouse to Button (Direction to move AWAY)
                        ...
                        // Repulsion strength (How far to jump)
                        ...
                        // Boundary Check (Clamp to Container Logic)
                    ...
                    // 1. Proximity Sensor (Mousemove on container)
                    container.addEventListener('mousemove', (e) => {{
                        ...
                        // Trigger repulsion if mouse gets closer than 100px
                        if (dist < 100) {{
                            triggerMove(e.clientX, e.clientY);
                        }}
                    }});

                    // 2. Click Handler (If user somehow manages to click it, move it anyway)
                    btn.addEventListener('click', (e) => {{
                        // Allow click on touch devices, otherwise prevent and move
                        if (!isTouchDevice) {{
                             e.preventDefault(); 
                             triggerMove(e.clientX, e.clientY);
                        }}
    ...
    projects_list_html = ""
    for p in projects:
        # FIX: Images are now 'h-64' on mobile and revert to sidebar style on desktop
        # FIX: Overlay is 'opacity-100' on mobile (always visible) and 'md:opacity-0' (hidden on PC until hover)
    ...
    <body class="bg-stone-50 text-stone-900 pt-20">
        ...
        <main class="max-w-5xl mx-auto px-6 py-20">
            ...
            <div class="relative py-10 max-w-4xl mx-auto">
                {experience_list_html}
            </div>
        </main>
        {get_footer()}
    ...
    for skill in competencies_list:
        ...
        <div class="skill-card p-4 border border-stone-200 rounded-lg bg-white hover:shadow-md flex items-center justify-center text-center">
            <h4 class="font-medium text-stone-800 text-sm">{skill}</h4>
        </div>
        """
                    ...
                    <div class="p-8 bg-stone-100 rounded-xl border-l-4 border-teal-600 h-full flex flex-col justify-center">
                        ...
                        <div class="mb-8">
                            <div class="flex justify-between items-baseline mb-2">
                                <h4 class="text-xl font-bold text-stone-900">BSc, Techno-Anthropology</h4>
                                <span class="text-sm font-mono text-stone-500">2022 – 2025</span>
                            </div>
                            <p class="text-teal-700 font-medium mb-2">Aalborg University, Copenhagen</p>
                            <p class="text-stone-600 text-sm mb-4">
                                <span class="lang-en">Weighted avg: 11.3 (Danish 7-scale) / ~4.0 GPA equivalent. Top grades in central projects.</span>
                                <span class="lang-da hidden">Vægtet gennemsnit: 11,3 (7-trins-skala). Topkarakterer på centrale projektforløb.</span>
                            </p>
                        ...
                        <div class="mb-8">
                            <div class="flex justify-between items-baseline mb-2">
                                <h4 class="text-xl font-bold text-stone-900">Film Production</h4>
                                <span class="text-sm font-mono text-stone-500">2021</span>
                            </div>
                            <p class="text-teal-700 font-medium mb-2">Askov Højskole</p>
                         ...
                         <div>
                            <div class="flex justify-between items-baseline mb-2">
                                <h4 class="text-xl font-bold text-stone-900">IBG / Democracy & Globalization</h4>
                 ...
                 <div class="grid md:grid-cols-2 gap-8">
                    <div class="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                         <div class="flex justify-between items-start mb-2">
                            ...
                            <span class="text-xs font-mono text-stone-500">Aug 2022 - Present</span>
                         </div>
                        <p class="text-stone-500 text-sm">Askov Højskole Elevforening</p>
                    </div>
                    <div class="bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                         <div class="flex justify-between items-start mb-2">
                            <span class="font-bold text-stone-900">Tutor</span>
                            <span class="text-xs font-mono text-stone-500">Sep 2023 - Dec 2023</span>
                         </div>
                        <p class="text-stone-500 text-sm mb-3">Aalborg University</p>
    ...
    print(f"1. Copy 'Profile.png' (Index) & 'Profile 2.jpeg' (Profile) into: {images_path}")
    print(f"2. Copy other PNG images and Logos into:                        {images_path}")
    print(f"3. Copy PDF files into:                                         {pdfs_path}")
    print(f"4. Check contact.html for Formspree ID")
    print("-" * 60)
